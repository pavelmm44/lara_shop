...
@section('content')
    ...
    <x-forms.auth-forms title="Login to account" action="{{ route('login.handle') }}" method="POST">
        @csrf
        <x-forms.text-input
            name="email"
            type="email"
        ...
        <x-slot:buttons>
            <div class="space-y-3 mt-5">
                <div class="text-xxs md:text-xs"><a href="{{ route('forgot') }}" class="text-white hover:text-white/70 font-bold">Forgot password?</a></div>
                <div class="text-xxs md:text-xs"><a href="{{ route('register') }}" class="text-white hover:text-white/70 font-bold">Signup</a></div>
            </div>
        </x-slot:buttons>
    </x-forms.auth-forms>
