        ...
        <div class="flex flex-col lg:flex-row gap-12 lg:gap-6 2xl:gap-8 mt-8">
            ...
            <aside class="basis-2/5 xl:basis-1/4">
                <form action="{{ route('catalog', $category) }}" class="overflow-auto max-h-[320px] lg:max-h-[100%] space-y-10 p-6 2xl:p-8 rounded-2xl bg-card">
                    ...
                    @if(request('filters'))
                        <div>
                            <a href="{{ route('catalog') }}" class="w-full !h-16 btn btn-outline">
                                Reset filters
                            </a>
                        </div>
                    @endif
                    ...
                    <div x-data="{ }" class="flex flex-col sm:flex-row sm:items-center gap-3">
                        ...
                        <form x-ref="sortForm">
                            ...
                            >
                                ...
                                <option @selected(request('sort') === 'price') value="price" class="text-dark">from cheap to expensive</option>
                                <option @selected(request('sort') === '-price') value="-price" class="text-dark">from expensive to cheap</option>
                                <option @selected(request('sort') === 'title') value="title" class="text-dark">title</option>
                            </select>
                        </form>
                    </div>
                </div>
